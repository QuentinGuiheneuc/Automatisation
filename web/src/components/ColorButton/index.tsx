...
export default function ColorButton({lightId, color, handleColor}: props) {
    ...
    return(
        <div className="relative">
            <div className={`h-4 w-4 rounded-full cursor-pointer`} style={{background: `${color}`}}onClick={handleClick}/>
            {isColorPicker &&
                <div className="absolute z-10 top-5 left-2">
                    <div className="fixed right-0 top-0 bottom-0 left-0" onClick={handleClick}/>
                    <ChromePicker
                        color={color}
                        disableAlpha={true}
